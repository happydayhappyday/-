...
+ (NSString *)appendWeekStringFromDate:(NSDate *)date withFormat:(NSString *)format{
     return [NSString stringWithFormat:@"%@ %@",[self dateString:date withFormat:format], [self wekdayStringFromDate:date]];
}
+ (NSString *)wekdayStringFromDate:(NSDate *)inputDate{
    NSArray *weekdays = [NSArray arrayWithObjects:[NSNull null],@"星期天",@"星期一",@"星期二",@"星期三",@"星期四",@"星期五",@"星期六", nil];
    NSCalendar *calendar = [[NSCalendar alloc]initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSTimeZone *timeZone = [[NSTimeZone alloc] initWithName:@"Asia/Shangehai"];
